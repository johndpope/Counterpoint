...
-(id)init
{
	...
	if (self)
	{
		...
		_finalResponse = [[NSString alloc] init];
	}
	return self;
}
...
-(void)scrobbleCPTrack:(CPTrack*)track
{
	...
	[parameterDictionary setObject:@"artist" forKey:[track artist]];
	[parameterDictionary setObject:@"track" forKey:[track title]];
	[parameterDictionary setObject:@"album" forKey:[track album]];
	
	NSTimeInterval duration = [[NSDate date] timeIntervalSince1970];
	duration = duration - ([[track durationMilliSeconds] intValue]/1000);
	[parameterDictionary setObject:@"timestamp" forKey:@(duration)];
	
	[parameterDictionary setObject:@"api_key" forKey:lastFMApiKey];
	[parameterDictionary setObject:@"sk" forKey:[[NSUserDefaults standardUserDefaults] objectForKey:@"lastfmSessionKey"]];
	[parameterDictionary setObject:@"method" forKey:@"track.scrobble"];
	[parameterDictionary setObject:@"format" forKey:@"json"];
...
-(NSString*)getFormURLEncodedStringWithParameters:(NSDictionary*)parameterDictionary
{
	...
	[parameterString appendFormat:@"&api_sig=%@", [self getLastFMApiSignatureWithParameters:parameterDictionary]];
...
-(void)sendRequestWithParameterDictionary:(NSDictionary*)parameterDictionary requestStage:(NSInteger)requestStage
{
	...
	NSURL* authenticationURL = [NSURL URLWithString:@"http://ws.audioscrobbler.com/2.0/"];
...
-(void)requestMobileSession
{
	...
	[parameterDictionary setObject:@"api_key" forKey:lastFMApiKey];
	[parameterDictionary setObject:@"method" forKey:@"auth.getMobileSession"];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	if ([self requestStage] == LastFMConnectionStageRequestMobileSession)
	{
        ...
        else
        {
			...
			[[NSUserDefaults standardUserDefaults] setObject:responseDict[@"key"] forKey:@"lastfmSessionKey"];
        }
	}
